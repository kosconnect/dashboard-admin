  ...
  fetch("https://kosconnect-server.vercel.app/api/facility/", {
    ...
    .then((data) => {
      ...
      data.forEach((facility, index) => {
        ...
        const facilityType = facility.type || "Tidak ada tipe";
        ...
        const tdTypeFasilitas = document.createElement("td");
        tdTypeFasilitas.textContent = facilityType;
        tr.appendChild(tdTypeFasilitas);

        const tdAksi = document.createElement("td");
        tdAksi.innerHTML = `
                <button class="btn btn-primary" onclick="showPopupEdit('${facilityId}', '${facilityName}', '${facilityType}')">
                    <i class="fas fa-edit"></i> Edit
                </button>
                <button class="btn btn-primary" onclick="showPopupDelete('${facilityId}')">
...
document
  .getElementById("formTambahFasilitas")
...
document
  .getElementById("formEditFasilitas")
  .addEventListener("submit", function (event) {
    event.preventDefault(); // Mencegah reload halaman saat form disubmit
    ...
    const namaFasilitas = document
      .getElementById("editNamaFasilitas")
      .value.trim();
    const typeFasilitas = document
      .getElementById("editTypeFasilitas")
      .value.trim();
...
function executeDelete() {
  ...
  fetch(
    `https://kosconnect-server.vercel.app/api/facility/${facilityIdToDelete}`,
    {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${jwtToken}`,
